...
function md5(password) {
    ...
    return cryptor.update(password).digest('hex');
}
/**
 * 签发token
 * @param {*} userId 
    ...
    try {
        let jwtSign = async function signToken() {
            return new Promise((resolve, reject) => {
                jwt.sign({ userId: userId, exp: getCurrentTime() + expireTime }, signKey, (err, token) => {
                    if (err) {
                        reject(err)
                    }
                    else {
        ...
        let tokenItem = {
            userId: userId,
            tokenId: md5(res.token),
            token: res.token,
            time: getCurrentTime()
        }
        cache.tokenCache[userId] = tokenItem;
        cache.tokenCache[tokenItem.tokenId] = tokenItem;
        //logger.trace("userId:" + userId + " 发放tokenId:" + tokenItem.tokenId + " 发放时间:" + tokenItem.time);
        result = { code: 1, message: "生成token成功", data: tokenItem.tokenId };
    }
    catch (err) {
        {
            result = { code: error.signError.code, message: error.signError.message + err.message };
        }
 ...
 * @param {*} callback 
 */
async function verify(userId, tokenId) {
    let result = {};
    if (!tokenId) {
    ...
    try {
        ...
        console.log(err);
        if (err.name == "TokenExpiredError" && (getCurrentTime() - tokenItem.time < expireTime)) {
            result = await sign(userId);

        }
        else {
            result = { code: error.tokenVerifyError.code, message: error.tokenVerifyError.message + err.name }
