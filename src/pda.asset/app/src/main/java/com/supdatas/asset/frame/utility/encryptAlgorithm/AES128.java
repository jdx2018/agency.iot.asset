package com.supdatas.fw.base.utility.encryptAlgorithm;
...
public class AES128 {
    ...
    //private static final String keyBytes = "ABC123";

    /**
     *加密
     */
    public static String encode(String keySeed, String content){
        //加密之后的字节数组,转成16进制的字符串形式输出
        StringBuilder sb = new StringBuilder();
        if (keySeed == null)
            keySeed = "";
        if (keySeed.length() > 16)
            sb.append(keySeed.substring(0, 16));
        sb.append(keySeed);
        for (int i = keySeed.length(); i < 16; ++i){
            sb.append(" ");
        }
        String newKey = sb.toString();
        return parseByte2HexStr(encrypt(content, newKey));
    }

    /**
     *解密
     */
    public static String decode(String keySeed, String content){
        ...
        String newKey = sb.toString();
        byte[] b = decrypt(parseHexStr2Byte(content), newKey);
        return new String(b);
    }
}