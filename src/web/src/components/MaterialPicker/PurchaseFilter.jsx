...
export default React.memo(({ onAssetPickerComplete }) => {
  ...
  React.useEffect(() => {
    ;(async () => {
      ...
      const res = await clientService.bill_purchase.queryBill_materialchuku()
  ...
  const handleCanfirm = async () => {
    ...
    const res = await clientService.asset_material.getMaterialList_purchase(selectedKeys_[0])
    if (res.code === 1) {
      global.$showMessage({
        type: 'success',
