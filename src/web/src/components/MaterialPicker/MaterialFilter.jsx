...
export default React.memo(({ onMaterialPickerComplete, getMaterialListFunc }) => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const res = await getMaterialListFunc.getMaterialList()
  ...
  const filterDataChange = React.useCallback(
    (data) => {
      dataRef.current = data
      setData(data.slice(0, 10));
    },
  ...
  const handleCanfirm = () => {
    ...
    onMaterialPickerComplete(dataRef.current.filter((material) => selectedKeys_.includes(material.materialId)));
    global.$hideModal();
  };
