...
export default React.memo(({ onAssetPickerComplete, getAssetListFunc }) => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      const res = await getAssetListFunc.getAssetList()
  ...
  const handleCanfirm = () => {
    ...
    onAssetPickerComplete(dataRef.current.filter((asset) => selectedKeys_.includes(asset.assetId)));
    global.$hideModal();
  };
