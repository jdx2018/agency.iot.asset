import React, { useState, useEffect, useRef, memo, useMemo, useCallback } from 'react'

import { Button, Space, Table, Pagination, Modal, InputNumber, Tooltip } from 'antd'

import { Card } from '@material-ui/core'

import PerfectScrollbar from 'react-perfect-scrollbar'

import LocalMaterialPickerDialogContent from './LocalMaterialPickerDialogContent'

import app_config from 'config/app'

import { chunk, cloneDeep } from 'lodash'

import AddMaterialDialog from './AddMaterialDialog'
import clientService from 'api/clientService'

const getTotalAmount = (price, qty) => (price ? price : 0) * (qty ? qty : 0)

const chunkobj2list = (chunkobj) => {
  return Object.values(chunkobj).reduce((acc, cur) => acc.concat(cur), [])
}

export default memo(({ value, type, onChange, pickerConfig = null }) => {
  const selectedRef = useRef({})
  const dataRef = useRef([])
  const [data, setData] = useState([])
  const [columns, setColumns] = useState([])

  const [selectedRowKeys, setSelectedRowKeys] = useState([])
  const [pageSize, setPageSize] = useState(10)
  const [current, setCurrent] = useState(1)
  const supplierMap = useRef(null)

  const rowSelection = {
    selectedRowKeys,
    onChange: (selectedRowKeys, selectedRows) => {
      selectedRef.current[current] = selectedRowKeys
      setSelectedRowKeys(selectedRef.current[current])
      onChange(dataRef.current.filter((_) => chunkobj2list(selectedRef.current).includes(_.materialId)))
    },
  }

  const tableMinWidth = useMemo(
    () =>
      columns.reduce((acc, cur) => {
        if (cur.width && typeof cur.width === 'number') {
          return acc + cur.width
        }
        return acc + app_config.defaultColumnWidth
      }, 0) + 50,
    [columns]
  )
  const isDisableRemoveBtn = Object.values(selectedRef.current).reduce((acc, cur) => acc.concat(cur), []).length === 0
  const tableRowClassName = (record, index) => {
    if (index % 2 === 0) {
      return 'bg-f7f8fa'
    }
  }
  const totalSelectedCount = Object.values(selectedRef.current).reduce((acc, cur) => acc.concat(cur), []).length

  const handleAddMaterial = (material) => {
    if (supplierMap.current)
      material.forEach((_) => {
        _.supplierName = supplierMap.current.find((__) => __.supplierId === _.supplierId)?.supplierName
      })
    const old_materialIdList = dataRef.current.map((v) => v.materialId)

    dataRef.current = [...dataRef.current, ...material.filter((v) => !old_materialIdList.includes(v.materialId))]
    setData(dataRef.current.slice((current - 1) * pageSize, current * pageSize))
  }

  const onPageChange = (page, pageSize) => {
    setCurrent(page)
    setData(dataRef.current.slice((page - 1) * pageSize, page * pageSize))
    if (selectedRef.current[page]) {
      setSelectedRowKeys(selectedRef.current[page])
    }
  }

  const addHandler = useCallback(async (material, cb) => {
    const res = await clientService.asset_material.addMaterial(material)
    if (res.code === 1) {
      cb()
      handleAddMaterial([material])
    } else {
      global.$showMessage({
        message: res.message,
        type: 'error',
        autoHideDuration: 5000,
      })
    }
  }, [])

  useEffect(() => {
    if (value) {
      dataRef.current = value
      setData(dataRef.current.slice(0, pageSize))
    }
  }, [pageSize])

  useEffect(() => {
    ;(async () => {
      if (pickerConfig) {
        let header
        header = pickerConfig.header

        if (header) {
          const columns = Object.keys(header).map((header_key) => {
            if (header_key === 'orderQty') {
              return {
                title: header[header_key].zh,
                dataIndex: header_key,
                width: header[header_key].width,
                render: (text, record) => (
                  <InputNumber
                    style={{
                      width: 80,
                    }}
                    // precision={2}
                    size='small'
                    // formatter={(value) => `${value} 件`}
                    min={0}
                    disabled={type === 'detail'}
                    value={text ? Number(text) : 0}
                    onChange={(val) => {
                      const selectedRowKeys = Object.values(selectedRef.current).reduce(
                        (acc, cur) => acc.concat(cur),
                        []
                      )

                      setData((c) => {
                        let c_copy = cloneDeep(c)
                        c_copy.forEach((item) => {
                          if (item.materialId === record.materialId) {
                            item.orderQty = val
                            dataRef.current.forEach(dataRefItem => {
                              if(item.materialId === dataRefItem.materialId) {
                                dataRefItem.orderQty = val
                                dataRefItem.amount = getTotalAmount(item.orderPrice, item.orderQty)
                              }
                            })
                          }
                          item.amount = getTotalAmount(item.orderPrice, item.orderQty)
                        })
                        return c_copy
                      })
                      if (selectedRowKeys.includes(record.materialId)) {
                        onChange(dataRef.current)
                      }
                    }}
                  />
                ),
              }
            }
            if (header_key === 'orderPrice') {
              return {
                title: header[header_key].zh,
                dataIndex: header_key,
                width: header[header_key].width,
                render: (text, record) => (
                  <InputNumber
                    style={{
                      width: 80,
                    }}
                    // precision={2}
                    size='small'
                    disabled={type === 'detail'}
                    // formatter={(value) => `¥ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
                    min={0}
                    value={text ? Number(text) : 0}
                    onChange={(val) => {
                      const selectedRowKeys = Object.values(selectedRef.current).reduce(
                        (acc, cur) => acc.concat(cur),
                        []
                      )

                      setData((c) => {
                        let c_copy = cloneDeep(c)
                        c_copy.forEach((item) => {
                          if (item.materialId === record.materialId) {
                            item.orderPrice = val
                            dataRef.current.forEach(dataRefItem => {
                              if(item.materialId === dataRefItem.materialId) {
                                dataRefItem.orderPrice = val
                                dataRefItem.amount = getTotalAmount(item.orderPrice, item.orderQty)
                              }
                            })
                          }
                          item.amount = getTotalAmount(item.orderPrice, item.orderQty)
                        })
                        return c_copy
                      })
                      if (selectedRowKeys.includes(record.materialId)) {
                        onChange(dataRef.current)
                      }
                    }}
                  />
                ),
              }
            }
            return {
              title: header[header_key].zh,
              dataIndex: header_key,
              width: header[header_key].width,
            }
          })
          setColumns(columns)
        }
      }
    })()
  }, [pickerConfig])

  useEffect(() => {
    // TODO
    if (pickerConfig && pickerConfig.rows) {
      dataRef.current = pickerConfig.rows.slice()
      setData(dataRef.current.slice((current - 1) * pageSize, current * pageSize))
      let obj = {}
      chunk(
        dataRef.current.map((v) => v.materialId),
        20
      ).forEach((v, index) => {
        obj[index + 1] = v
      })
      selectedRef.current = obj
      setSelectedRowKeys(selectedRef.current[current] ? selectedRef.current[current] : [])
    }
  }, [pageSize, pickerConfig])

  useEffect(() => {
    if (type === 'create') {
      dataRef.current = []
      setData(dataRef.current.slice(0, pageSize))
    }
  }, [pageSize, type])

  return (
    <>
      {type !== 'detail' && (
        <Space
          spacing={8}
          style={{
            marginBottom: 10,
            marginTop: 10,
          }}
        >
          <Button
            type='primary'
            onClick={() => {
              global.$showModal({
                zIndex: 9998,
                content: <LocalMaterialPickerDialogContent onAddMaterial={handleAddMaterial} />,
                title: '新增耗材',
                width: '948px',
              })
            }}
          >
            添加
          </Button>
          <Tooltip title='新增耗材会添加一个耗材进本地候选耗材列表，同时把该耗材信息同步到后台'>
            <Button
              onClick={() => {
                supplierMap.current = null
                global.$showModal({
                  zIndex: 9998,
                  width: 948,
                  content: (
                    <AddMaterialDialog
                      addHandler={addHandler}
                      type='create'
                      setSupplierMap={(data) => {
                        supplierMap.current = data
                      }}
                    />
                  ),
                  title: `新增耗材`,
                })
              }}
            >
              新增耗材
            </Button>
          </Tooltip>
          <Button
            type='primary'
            disabled={isDisableRemoveBtn}
            onClick={() => {
              Modal.confirm({
                title: '警告',
                content: `确认删除选中的耗材信息？`,
                onOk: async () => {
                  dataRef.current = dataRef.current.filter(
                    (v) => !chunkobj2list(selectedRef.current).includes(v.materialId)
                  )
                  setData(dataRef.current.slice((current - 1) * pageSize, current * pageSize))
                  Object.keys(selectedRef.current).forEach((v) => {
                    selectedRef.current[v] = []
                  })
                  setSelectedRowKeys(selectedRef.current[current])
                  onChange(dataRef.current)
                },
              })
            }}
          >
            删除
          </Button>
        </Space>
      )}
      <Card
        style={{
          boxShadow: 'none',
          border: '1px solid #f0f0f0',
          // padding: 5,
          marginBottom: 10,
        }}
      >
        <PerfectScrollbar style={{ width: '100%', height: 210 }}>
          <div
            style={{
              minWidth: tableMinWidth,
            }}
          >
            <Table
              sticky
              rowKey='materialId'
              rowClassName={tableRowClassName}
              size='middle'
              rowSelection={type !== 'detail' ? rowSelection : false}
              pagination={false}
              columns={columns}
              dataSource={data}
            />
          </div>
        </PerfectScrollbar>
      </Card>
      <div
        style={{
          paddingLeft: 10,
          paddingRight: 10,
          boxShadow: 'none',
          display: 'flex',
          justifyContent: 'space-between',
          alignItems: 'center',
        }}
      >
        <div>
          共&nbsp;
          <span
            style={{
              fontSize: 16,
              color: '#00bcd4',
              backgroundColor: '#fff!important',
            }}
          >
            {dataRef.current.length}&nbsp;
          </span>
          条数据
          {type === 'create' && (
            <span>
              ，当前选中&nbsp;
              <span
                style={{
                  fontSize: 16,
                  color: '#00bcd4',
                }}
              >
                {totalSelectedCount}
              </span>
              &nbsp; 条
            </span>
          )}
        </div>
        <div>
          <Pagination
            hideOnSinglePage
            onChange={onPageChange}
            current={current}
            total={dataRef.current.length}
            pageSize={pageSize}
          />
        </div>
      </div>
    </>
  )
})
