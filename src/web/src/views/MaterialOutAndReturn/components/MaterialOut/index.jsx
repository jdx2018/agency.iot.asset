...
export default React.memo(() => {
  ...
  const refreshData = React.useCallback(async (filter = null) => {
    ...
    const res = await clientService.bill_chuku_material.queryBill(filter_)
    setLoading(false)
    if (res.code === 1) {
            ...
            return (
              <a
                onClick={async () => {
                  setOpen(true)
                  setCurNo(text)
                  const res = await clientService.bill_chuku_material.queryBillDetail(text)
                  if (res.code === 1) {
                    setDialogData(res.data)
                  } else {
                    setDialogData({ billMain: {}, billDetail: {} })
                    global.$showMessage({
                      message: res.message,
                      type: 'error',
