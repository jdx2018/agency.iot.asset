...
import { PlusOutlined, DeleteOutlined, AlignLeftOutlined } from '@ant-design/icons';
  ...
  ({
    ...
    const handleEdit = () => {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res = await clientService.bill_chuku_material.deleteBill(billNo);
          if (res.code === 1) {
            global.$showMessage({
    ...
    return (
      <>
        ...
        >
          <Space size={10}>
            <HideButtonWithAuth funcId='haocai_chuku_add'>
              <Button
                type='primary'
                icon={<PlusOutlined />}
                onClick={async () => {
                  setFormType('create');
                  setOpen(true);
                  const res = await clientService.bill_chuku_material.getBillMainTemplate();
                  if (res.code === 1) {
            ...
            <HideButtonWithAuth funcId='haocai_chuku_delete'>
