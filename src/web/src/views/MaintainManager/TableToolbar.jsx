  ...
  ({
    ...
    const handleEdit = () => {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res = await clientService.bill_weixiu.deleteBill(billNo);
          if (res.code === 1) {
            global.$showMessage({
    ...
    return (
      <>
        ...
        >
          <Space size={10}>
            <HideButtonWithAuth funcId='weixiu_add'>
              <Button
                type='primary'
                icon={<PlusOutlined />}
                onClick={async () => {
                  setFormType('create');
                  setOpen(true);
                  const res = await clientService.bill_weixiu.getBillMainTemplate();
                  if (res.code === 1) {
            ...
            <HideButtonWithAuth funcId='weixiu_shenhe'>
              <Button
                ...
                onClick={async () => {
                  Modal.confirm({
                    ...
                    onOk: async () => {
                      const billNo = selectedRowKeys[0];
                      const res = await clientService.bill_weixiu.checkBill(billNo);
                      if (res.code === 1) {
                        global.$showMessage({
            ...
            <HideButtonWithAuth funcId='weixiu_edit'>
              <Button
                // icon={ }
                disabled={selectedRowKeys.length !== 1}
                onClick={async () => {
                  setFormType('edit');
                  setOpen(true);
                  const res = await clientService.bill_weixiu.queryBillDetail(selectedRowKeys[0]);
                  if (res.code === 1) {
                    setDialogData(res.data);
                  } else {
                    global.$showMessage({
            ...
            <HideButtonWithAuth funcId='weixiu_delete'>
