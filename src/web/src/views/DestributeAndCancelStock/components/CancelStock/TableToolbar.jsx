  ...
  ({
    ...
    const handleEdit = () => {
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const res = await await clientService.bill_tuiku.deleteBill(billNo);
          if (res.code === 1) {
            global.$showMessage({
    ...
    return (
      <>
        ...
        >
          <Space size={10}>
            <HideButtonWithAuth funcId='tuiku_add'>
              <Button
                type='primary'
                icon={<PlusOutlined />}
                onClick={async () => {
                  setFormType('create');
                  setOpen(true);
                  const res = await clientService.bill_tuiku.getBillMainTemplate();
                  if (res.code === 1) {
            ...
            <HideButtonWithAuth funcId='tuiku_shenhe'>
              <Button
                ...
                onClick={async () => {
                  Modal.confirm({
                    ...
                    onOk: async () => {
                      const billNo = selectedRowKeys[0];
                      const res = await clientService.bill_tuiku.checkBill(billNo);
                      if (res.code === 1) {
                        global.$showMessage({
            ...
            <HideButtonWithAuth funcId='tuiku_delete'>
