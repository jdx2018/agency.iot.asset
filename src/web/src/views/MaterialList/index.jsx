...
export default React.memo(() => {
	...
	const refreshData = useCallback(async (filter = null) => {
		...
		setLoading(true)
		const res = await clientService.asset_material.getMaterialList(f__)
		setLoading(false)
		if (res.code === 1) {
	...
	const deleteHandler = useCallback(
		async ({ materialId }, cb) => {
			const res = await clientService.asset_material.deleteMaterial(materialId)
			if (res.code === 1) {
				cb()
				refreshData()
			} else {
	...
	const updateHandler = useCallback(
		async (material, cb) => {
			const material_copy = lodash.cloneDeep(material)
			const res = await clientService.asset_material.updateMaterial(material_copy.materialId, material_copy)
			if (res.code === 1) {
				cb()
				refreshData()
			} else {
