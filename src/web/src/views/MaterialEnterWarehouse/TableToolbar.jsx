  ...
  ({
    ...
    return (
      <>
        ...
        >
          <Space size={10}>
            <HideButtonWithAuth funcId='haocai_enter_add'>
              <Button
                type='primary'
                icon={<PlusOutlined />}
                onClick={async () => {
                  setFormType('create');
                  setOpen(true);
                  const res = await clientService.bill_ruku_material.getBillMainTemplate();
                  if (res.code === 1) {
            ...
            <HideButtonWithAuth funcId='haocai_enter_edit'>
              <Button
                disabled={
                  selectedRowKeys.length !== 1 ||
                  originData.find((v) => selectedRowKeys[0] === v.billNo).status !== '待入库'
                }
                icon={<EditOutlined />}
                onClick={async () => {
                  setFormType('edit');
                  setOpen(true);
                  const res = await clientService.bill_ruku_material.queryBillDetail(selectedRowKeys[0]);
                  if (res.code === 1) {
                    setDialogData(res.data);
                    // setGetMaterialListFunc(res.method)
                  } else {
            ...
            <HideButtonWithAuth funcId='haocai_enter_shenhe'>
              <Button
                ...
                onClick={async () => {
                  Modal.confirm({
                    ...
                    onOk: async () => {
                      ...
                      const res = await clientService.bill_ruku_material.checkBill(billNo);
                      if (res.code === 1) {
                        global.$showMessage({
            ...
            <HideButtonWithAuth funcId='haocai_enter_quxiaoruku'>
              <Button
                ...
                onClick={async () => {
                  Modal.confirm({
                    ...
                    onOk: async () => {
                      const billNo = selectedRowKeys[0];
                      const res = await clientService.bill_ruku_material.cancelBill(billNo);
                      if (res.code === 1) {
                        global.$showMessage({
                          message: `取消入库${billTitle}单成功`,
                          type: 'success',
                        });
                        setSelectedRowKeys(null);
                        refreshData();
            ...
            <HideButtonWithAuth funcId='haocai_enter_delete'>
              <Button
                disabled={selectedRowKeys.length !== 1}
                icon={<DeleteOutlined />}
                type='danger'
                onClick={() => {
                  Modal.confirm({
                    ...
                    onOk: async () => {
                      const billNo = selectedRowKeys[0];
                      const res = await clientService.bill_ruku_material.deleteBill(billNo);
                      if (res.code === 1) {
                        global.$showMessage({
