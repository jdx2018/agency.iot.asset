...
export default React.memo(() => {
  ...
  const refreshData = React.useCallback(async (filter = null) => {
    ...
    const res = await clientService.bill_ruku_material.queryBill(filter_)
    setLoading(false)
    if (res.code === 1) {
          ...
          column_.render = (text, row, index) => {
            return (
              <a
                onClick={async () => {
                  setOpen(true)
                  const res = await clientService.bill_ruku_material.queryBillDetail(text)
                  if (res.code === 1) {
